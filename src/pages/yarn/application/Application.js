...
export default class Tenant extends React.Component {
    ...
    componentDidMount() {
        ...
        this.getAppList(this.context.clusterValue);
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        ...
        if (nextContext.clusterRequestError) { // 请求集群有误
            ...
            this.getAppList(newClusterValue);
    ...
    // 切换队列值
    onAppChange = (value) => {
        if (value === this.state.poolName) return;
        this.setState(update(this.state, {
            poolName: { $set: value },
            memoryTopNTrend: {
                loading: { $set: true },
    ...
    exportTopNDetail = (type) => () => {
        ...
        switch (type) {
            case 'memory':
                ...
                url = 'exportMemoryToExcel';
                break;
            case 'cpu':
                data = this.cpuTopNDetailSource;
                url = 'exportCpuToExcel';
                break;
